...
class Server(models.Model):
    """
    Represents import source server. This can be one of following cases:
    
    **Remote machine** - ``url`` attribute is set and is expected as Atom feed
    source.
    **Different site in same database** - ``category`` attribute is used
    to find out from where to import. 
    """
    ...
    def regenerate(self):
        """Returns HTML link to inialize a fetch"""
        return mark_safe(u'<a href="%s/fetch/">%s - %s</a>' % (self.id, _('Fetch'), self.title))
    regenerate.allow_tags = True

    def get_imports_by_time(self):
        """Returns imported items sorted descending by time of last change"""
        return self.serveritem_set.order_by("-updated")

    def get_imports_by_priority(self):
        """Returns imported items sorted descending by priority and time of 
        last change"""
        return self.serveritem_set.order_by("-priority", "-updated")

    def get_from_category(self):
        """
        Runs import from other Ella application.
        
        Basic prerequisite is that the app must be placed on same database. To
        find out what to import ``self.category`` attribute is used.
        """
        if not self.category:
            raise ValueError, "You need to set my ``category`` attribute prior " \
                "to running this method"

        models = [Article]
        articles = Listing.objects.get_listing(category=self.category,
            count=ELLA_IMPORT_COUNT, mods=models)
        output = {
            'status':200,
            'entries':[]
        }
    ...
    def get_from_feed(self):
        ...
        try:
            ...
            raise ImproperlyConfigured, 'You must have feedparser installed in ' \
                'order to use automated imports.'
    ...
    def fetch(self):
        """
        Runs the item fetch from the category or feed (based on self.category)
        and saves them into ServerItem.
        """
        if self.category:
        ...
        if output['status'] == 200 and importlen > 0:
            ...
            for index in range(importlen):
                ...
                if not created and si.priority >= 0:
                    # unwanted import item has negative priority - never
                    # more update priority
...
class ServerItem(models.Model):
    """
    Used to keep track of imported items from ``Server``. Represents
    one item from the remote source.
    """
    ...
    photo_url = models.URLField(_('Image URL'), verify_exists=False,
        max_length=400, blank=True)
    photo = models.ForeignKey(_photo_model(), blank=True, null=True,
